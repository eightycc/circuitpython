...
static mp_obj_t ramlog_ramlog_init(mp_obj_t self_in, mp_obj_t size) {
    if (!ram_log_init(mp_obj_get_int(size))) {
...
static mp_obj_t ramlog_ramlog_get_log_enable(mp_obj_t self_in) {
    bool log_enable = ram_log_get_enable();
...
static mp_obj_t ramlog_ramlog_set_log_enable(mp_obj_t self_in, mp_obj_t log_enable) {
    ramlog_ramlog_obj_t *self = MP_OBJ_TO_PTR(self_in);
    ram_log_enable(mp_obj_is_true(log_enable));
...
static mp_obj_t ramlog_ramlog_get_wrap_enable(mp_obj_t self_in) {
    bool wrap_enable = ram_log_get_wrap_enable();
...
static mp_obj_t ramlog_ramlog_set_wrap_enable(mp_obj_t self_in, mp_obj_t wrap_enable) {
    ram_log_wrap_enable(mp_obj_is_true(wrap_enable));
    return mp_const_none;
}
...
static void init_read_pos(ramlog_ramlog_obj_t *self) {
    ...
    self->read_limit = _ram_log->log_buf_sz;
    pos_oldest_complete_message(self);
    self->read_limit = (_ram_log->wrapped)?
        _ram_log->log_buf_sz - self->read_start_offset :
        _ram_log->next_idx;
}
...
inline static mp_uint_t physical_read_pos(ramlog_ramlog_obj_t *self) {
    if (_ram_log->wrapped && _ram_log->wrap_enabled) {
        return ((_ram_log->next_idx + self->read_start_offset + self->read_pos) < _ram_log->log_buf_sz)?
               _ram_log->next_idx + self->read_start_offset + self->read_pos :
               _ram_log->next_idx + self->read_start_offset + self->read_pos - _ram_log->log_buf_sz;
    }
...
inline static uint64_t decode_timestamp(ramlog_ramlog_obj_t *self) {
    ...
    while (bits > 0) {
        ...
        uint8_t ts_byte = _ram_log->log_buf[physical_read_pos(self)];
...
inline static uint8_t ramlog_getc(ramlog_ramlog_obj_t *self) {
    ...
    uint8_t log_byte = _ram_log->log_buf[physical_read_pos(self)];
    increment_read_pos(self);
    return log_byte;
}
...
static void pos_oldest_complete_message(ramlog_ramlog_obj_t *self) {
    ...
    // self->read_start_offset = _ram_log->next_idx;
...
// Read the next byte(s) from the current ram_log_t including the RAM log buffer.
// If there is no current ram_log_t or logging is active, return 0 indicating EOF.
static mp_uint_t ramlog_ramlog_read(mp_obj_t self_in, void *buf, mp_uint_t size, int *errcode) {
    ramlog_ramlog_obj_t *self = MP_OBJ_TO_PTR(self_in);

    if (_ram_log == NULL || _ram_log->logging_enabled || self->read_eof) {
        return 0;
    }
...
static mp_uint_t ramlog_ramlog_write(mp_obj_t self_in, const void *buf, mp_uint_t size, int *errcode) {
    ...
    while (remaining-- > 0) {
        ...
        if (self->write_pos >= RAM_LOG_WRITE_BUF_SIZE) {
            ...
            ram_log_printf("%.*s", self->write_pos, self->write_buf);
            self->write_pos = 0;
        }
        ...
        case MP_STREAM_SEEK: {
            ...
            switch (s->whence) {
                case MP_SEEK_CUR:
                    ...
                    break;
                case MP_SEEK_END:
                    ref = sizeof(ram_log_t) + _ram_log->log_buf_sz;
                    break;
            }
